    ...
    <div class="container-select-room">
        ...
        <div class="room-list">
            @forelse ($ruangans as $r)
                <a href="
          @if (request()->routeIs('display.tampilan.select')) {{ route('display.tampilan', $r->id_ruangan) }}
          @else
            {{ route('display.jadwal', $r->id_ruangan) }} @endif
        "
                    class="room-btn" target="_blank" rel="noopener noreferrer">
                    {{ $r->nama }}
                </a>
            @empty

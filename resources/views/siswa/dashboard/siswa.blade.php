                        ...
                        @else
                            @if ($absenHariIni)
                                <div class="d-flex align-items-center">
                                    ...
                                    <small class="text-muted">Jam:
                                        {{ \Carbon\Carbon::parse($absenHariIni->created_at)->format('H.i') }}</small>
                                </div>
                            @elseif($izinAktif)
                                <div class="d-flex align-items-center">
                                    <span class="badge bg-warning text-dark me-2"> {{ ucfirst($izinAktif->jenis) }}</span>
                                    <small class="text-muted">{{ $izinAktif->tanggal_mulai }} –
                                        {{ $izinAktif->tanggal_selesai }}</small>
                                </div>
                            @else
                                <div class="d-flex align-items-center">
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-3">
                        ...
                        <div class="card-body">
                            ...
                            <p class="text-muted mb-1">{{ $izinAktif->tanggal_mulai }} – {{ $izinAktif->tanggal_selesai }}
                            </p>

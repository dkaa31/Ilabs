...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            <div class="col-12">
                <h2 class="text-gray-900 fw-bold">Selamat pagi, {{ auth()->user()->nama }}!</h2>
                @if ($kelasBinaan)
                        ...
                        <div class="card-body">
                            @if ($jadwalSaatIni)
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <small class="text-muted">
                                        Sisa waktu:
                                        {{ now()->diffInMinutes(\Carbon\Carbon::parse($jadwalSaatIni->jam_selesai), false) }}
                                        menit
                                    </small>
                                </div>
                            ...
                            @else
                                ...
                                @if ($jadwalHariIni->isNotEmpty())
                                    ...
                                    <h6 class="mb-1">{{ $jamMulai }} – {{ $jadwalBerikutnya->mapel->nama ?? '-' }}
                                    </h6>
                                    <p class="text-muted mb-2">
                                        {{ $jadwalBerikutnya->kelas->nama ?? '-' }} •
                                        {{ $jadwalBerikutnya->ruangan->nama ?? '-' }}
                                    </p>
                                @else
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-3">
                        <div class="card-header bg-light py-2">
                            <h6 class="mb-0"><i class="fas fa-file-alt me-2"></i>Izin Menunggu
                                ({{ $izinMenunggu->count() }})</h6>
                        </div>
                                ...
                                @foreach ($izinMenunggu as $izin)
                                    <li class="list-group-item d-flex justify-content-between align-items-center px-3 py-2">
                                        <div>
                                            ...
                                            <br>
                                            <small class="text-muted">{{ ucfirst($izin->jenis) }} •
                                                {{ $izin->tanggal_mulai }}</small>
                                        </div>
                                        <div>
                                            ...
                                            <form action="{{ route('izin.reject', $izin) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                    ...
                    <div class="card border-0 shadow-sm rounded-3">
                        <div class="card-header bg-light py-2">
                            <h6 class="mb-0">
                                ...
                                Kehadiran Kelas {{ $kelasBinaan->nama }} -
                                {{ $tanggalSekarang->translatedFormat('l, j F Y') }}
                            </h6>
                        </div>
                        <div class="card-body">
                            <!-- Hadir -->
        ...
        @if ($jadwalHariIni->count() > 0)
            <div class="row">
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-3">
                        <div class="card-header bg-light py-2">
                            <h6 class="mb-0"><i class="fas fa-calendar-day me-2"></i>Jadwal
                                {{ $tanggalSekarang->translatedFormat('l') }}</h6>
                        </div>
                        <div class="card-body p-0">
                            <ul class="list-group list-group-flush">

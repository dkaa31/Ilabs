        ...
        <form action="{{ route('jadwal.update', $jadwal) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="status" class="form-select" id="status" required>
                    ...
                    <option value="Istirahat" {{ (old('status') ?? $jadwal->status) == 'Istirahat' ? 'selected' : '' }}>
                        Istirahat</option>
                </select>
            </div>
    ...
    @push('scripts')
        <script>
            document.getElementById('status').addEventListener('change', function() {
                ...
                const selects = fields.querySelectorAll(
                    'select[name="guru_id"], select[name="mapel_id"], select[name="kelas_id"]');
